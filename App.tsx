...
const App: React.FC = () => {
  ...
  const handleToggleKaryTask = useCallback(async (taskId: string) => {
  ...
  const handleUpdateLog = useCallback(async (logId: string, updates: Partial<Log>) => {
    try {
  ...
  const handleUpdateLogTemplate = useCallback(async (templateId: string, updates: Partial<LogTemplate>) => {
    try {
      await updateLogTemplateInFirestore(templateId, updates);
      setLogTemplates(prev => prev.map(t => t.id === templateId ? { ...t, ...updates } : t));
    } catch (error) {
  ...
  const handleUpdateListFolder = useCallback(async (folderId: string, updates: Partial<ListFolder>) => {
    try {
  ...
  const handleUpdateTagFolder = useCallback(async (folderId: string, updates: Partial<TagFolder>) => {
    try {
  ...
  const handleDeleteHabitLog = useCallback(async (habitId: string, date: Date) => {
    try {
      ...
      const logToDelete = habitLogs.find(l => l.habitId === habitId && l.date === date.toISOString().split('T')[0]);
      if (logToDelete) {
            ...
            return <KaryView 
                        ...
                        onDeleteTask={async (taskId) => {
                          try {
                            ...
                            if (taskToDelete) {
                              const childrenIds = new Set(tasks.filter(t => t.parentId === taskId).map(t => t.id));
