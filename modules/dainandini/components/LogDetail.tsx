                            ...
                            return (
                                <li key={focus.id}>
                                    ...
                                    >
                                        ...
                                        {log.focusId === focus.id && <Icons.CheckSquareIcon className="w-4 h-4 text-blue-600" />}
                                    </button>
                                </li>
                            );
                        })}
        ...
        <div className="flex-1 bg-white/90 p-6 flex flex-col h-full">
            <header className="flex justify-between items-center mb-6 pb-4 border-b border-gray-200 flex-shrink-0">
                <div className="flex items-center gap-3 flex-wrap">
                    ...
                    >
                        ...
                        {new Date(log.logDate).toLocaleString('en-US', { month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                    </button>
                </div>
                <button onClick={() => onDeleteLog(log.id)} className="p-2 hover:bg-gray-100 rounded-full" aria-label="Delete log">
            ...
            <div className="flex-1 overflow-y-auto pr-2 -mr-2">
                <div className="flex items-start gap-3 mb-6">
                    ...
                    <div className="flex-1 min-w-0">
                        {isEditingTitle ? (
                            ...
                            <h1 onClick={() => setIsEditingTitle(true)} className="text-2xl font-bold text-gray-800 cursor-text">{log.title}</h1>
                        )}
                    </div>
                </div>
                ...
                {log.logType === LogType.CHECKLIST && (
                    <div className="mt-4">
                        <h3 className="font-semibold text-gray-600 text-sm uppercase tracking-wider mb-3">Checklist</h3>
                        <ul className="space-y-2">
                            {checklist.map(item => (
                                <li key={item.id} className="flex items-center gap-3 p-1 rounded-md hover:bg-gray-100">
                                    <Checkbox checked={item.completed} onChange={() => handleToggleChecklistItem(item.id)} />
                                    <span className={`flex-1 text-sm ${item.completed ? 'text-gray-400 line-through' : 'text-gray-700'}`}>{item.text}</span>
                                </li>
                            ))}
                        </ul>
                        <form onSubmit={handleAddChecklistItem} className="mt-2">
                            ...
                            <input type="text" value={newChecklistItem} onChange={(e) => setNewChecklistItem(e.target.value)} placeholder="Add a checklist item" className="w-full pl-9 pr-3 py-2 bg-gray-100/70 border-2 border-transparent focus:border-blue-500 focus:bg-white focus:shadow-sm rounded-md outline-none transition" />
                            </div>
                        </form>
                    </div>
                )}
