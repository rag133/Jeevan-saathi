...
const CalendarView: React.FC<CalendarViewProps> = ({ 
    ...
    const groupedCalendarLogs = useMemo(() => {
        ...
        filteredLogs.forEach(log => {
            ...
            if (!groups.has(groupName)) { // Safeguard for logs with deleted foci
    ...
    const handlePrevWeek = () => setViewDate(prev => {
        const newDate = new Date(prev);
        newDate.setDate(prev.getDate() - 7);
        return newDate;
    });
    const handleNextWeek = () => setViewDate(prev => {
        const newDate = new Date(prev);
        newDate.setDate(prev.getDate() + 7);
        return newDate;
    });
        ...
        <div className="flex flex-col h-full">
                 ...
                 <div className="flex justify-between items-center">
                    ...
                    <div className="flex items-center space-x-2 text-gray-500">
                        ...
                        <div className="relative" ref={datePickerRef}>
                            <button onClick={() => setIsDatePickerOpen(o => !o)} className="p-1.5 rounded-md hover:bg-gray-100" aria-label="Open date picker">
                        ...
                        <button onClick={handleGoToToday} title="Go to Today" className="px-3 py-1 text-sm border border-gray-300 rounded-md hover:bg-gray-100">Today</button>
                        <button onClick={handlePrevWeek} className="p-1 rounded-full hover:bg-gray-100"><Icons.ChevronLeftIcon className="w-5 h-5" /></button>
                        <button onClick={handleNextWeek} className="p-1 rounded-full hover:bg-gray-100"><Icons.ChevronRightIcon className="w-5 h-5" /></button>
                         <button
            ...
            <div className="p-4 pb-2">
                <div className="grid grid-cols-7 gap-2">
                    {weekGrid.map((day, i) => {
                        ...
                        let buttonClass = "w-full text-center p-2 rounded-lg transition-colors duration-150 border-2";
                        ...
                        return (
                            <div key={i}>
                                <button onClick={() => onDateSelect(day)} className={buttonClass}>
                                    <span className="text-xs uppercase text-gray-400 font-semibold">{day.toLocaleDateString('en-US', { weekday: 'short' })}</span>
                                    <span className={`block text-xl mt-1 font-bold ${isSelected ? 'text-white' : 'text-gray-800'}`}>{day.getDate()}</span>
                                    {hasLogs && <div className={`w-1.5 h-1.5 rounded-full mt-1.5 mx-auto ${isSelected ? 'bg-white' : 'bg-blue-500'}`}></div>}
                                </button>
                            </div>
                        );
                    })}
            ...
            <div className="flex-1 overflow-y-auto px-4 pb-4 border-t border-gray-200">
                <h2 className="text-lg font-semibold text-gray-700 my-4 px-2 sticky top-0 bg-white/90 backdrop-blur-sm py-2 z-10">
                    {selectedDate ? `Logs for ${new Date(selectedDate).toLocaleDateString('en-US', { month: 'long', day: 'numeric' })}` : 'Select a date to see logs'}
                </h2>
                {calendarViewMode === 'timeline' ? (
                     filteredLogs.length > 0 ? (
                        ...
                        selectedDate && <div className="text-center py-10 text-gray-500"><p>No logs on this day.</p></div>
                    )
                ) : (
                                ...
                                return (
                                    <div key={groupTitle}>
                                        <div className="flex items-center gap-2.5 mb-3 px-2">
                                            {focusForGroup && <IconComponent className={`w-5 h-5 text-${focusForGroup.color || 'gray-500'}`} />}
