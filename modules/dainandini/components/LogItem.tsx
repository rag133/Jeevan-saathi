    ...
    <ul className="space-y-2 mt-2">
      {items.map((item) => (
        <li key={item.id} className="flex items-center gap-3">
          <Checkbox checked={item.completed} onChange={() => {}} ariaLabel={item.text} disabled />
          <span
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3">
                ...
                <h3
                  className={`text-md font-semibold ${log.completed ? 'text-gray-400 line-through' : 'text-gray-800'}`}
                >
                  {log.title}
                </h3>
              </div>
              ...
              {log.description && (
                <p className="text-sm text-gray-600 mt-1 whitespace-pre-wrap truncate max-w-prose">
                  {log.description}
                </p>
              )}
