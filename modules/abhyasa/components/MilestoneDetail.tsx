...
const HabitCard: React.FC<{ habit: Habit }> = ({ habit }) => {
  ...
  return (
    <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 flex items-center gap-4 hover:bg-gray-100 transition-colors">
      ...
      <span
        className={`px-2 py-0.5 text-xs font-semibold rounded-full ${statusDetails.colorClass} flex-shrink-0`}
      >
        {habit.status}
      </span>
    </div>
...
const MilestoneDetail: React.FC<MilestoneDetailProps> = (props) => {
  ...
  const milestoneLogs = useMemo(() => {
    if (!milestone) return [];
    return allLogs
      .filter((log) => log.milestoneId === milestone.id)
  ...
  const renderPopup = () => {
    ...
    return (
      <div ref={popupRef} className="shadow-lg" style={popupStyle}>
        {popup === 'date' && (
          <DateTimePicker
            currentDate={milestone.targetEndDate ? new Date(milestone.targetEndDate) : null}
              ...
              >
                ...
                <span>
                  {milestone.targetEndDate
                    ? `Target: ${new Date(milestone.targetEndDate).toLocaleDateString()}`
                    : 'Set Target Date'}
                </span>
              </button>
            </div>
        ...
        <div className="mb-8">
          <div className="flex justify-between items-center mb-3">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => onAddHabit(milestone.id)}
                className="flex items-center gap-1.5 px-3 py-1 text-xs font-semibold text-blue-700 bg-blue-100 rounded-md hover:bg-blue-200 transition-colors"
              >
                <Icons.PlusIcon className="w-3.5 h-3.5" /> Add New
              </button>
              <button
                onClick={() => onLinkHabit(milestone.id)}
                className="flex items-center gap-1.5 px-3 py-1 text-xs font-semibold text-gray-700 bg-gray-100 rounded-md hover:bg-gray-200 transition-colors"
              >
          ...
          <div className="p-4 bg-gray-50 rounded-lg space-y-3 border">
            {milestoneLogs.length > 0 ? (
              ...
              <p className="text-sm text-gray-500 text-center py-2">
                No journal entries for this milestone yet.
              </p>
            )}
          </div>
        </div>
