...
const goalStatusItems: {id: GoalStatus, name: string, color: string, icon: keyof typeof Icons}[] = [
    { id: GoalStatus.NOT_STARTED, name: 'Not Started', color: 'text-gray-500', icon: 'SunriseIcon' },
    { id: GoalStatus.IN_PROGRESS, name: 'In Progress', color: 'text-blue-500', icon: 'RepeatIcon' },
    { id: GoalStatus.COMPLETED, name: 'Completed', color: 'text-green-500', icon: 'CheckSquareIcon' },
...
const GoalDetail: React.FC<GoalDetailProps> = (props) => {
    const { goal, milestones, habits, habitLogs, allFoci, allLogs, onUpdateGoal, onDeleteGoal, onAddMilestone, onLinkMilestone, onAddHabit, onLinkHabit, onAddHabitToMilestone, onLinkHabitToMilestone, onEditGoal, onOpenLogModal } = props;
    ...
    const triggerRefs = {
     ...
     const goalLogs = useMemo(() => {
        if (!goal) return [];
        return allLogs.filter(log => log.goalId === goal.id).sort((a,b) => b.logDate.getTime() - a.logDate.getTime());
    }, [goal, allLogs]);
    ...
    const handleDelete = () => {
        if(window.confirm(`Are you sure you want to delete the goal "${goal.title}"? This will also delete all of its milestones.`)) {
    ...
    const renderPopup = () => {
        ...
        return (
            <div ref={popupRef} className="shadow-lg" style={popupStyle}>
                {popup === 'date' && <DateTimePicker currentDate={goal.targetEndDate ? new Date(goal.targetEndDate) : null} onSelect={(date) => handleUpdate({ targetEndDate: date })} onClear={() => handleUpdate({ targetEndDate: undefined })} onClose={() => setPopup(null)} />}
                {popup === 'status' && (
                ...
                {popup === 'more' && (
                     <ul className="bg-white rounded-lg shadow-xl border border-gray-200 w-48 overflow-y-auto">
                        <li><button onClick={() => { onEditGoal(goal); setPopup(null); }} className="w-full text-left flex items-center gap-3 px-3 py-2 text-sm text-gray-700 hover:bg-gray-100"><Icons.Edit3Icon className="w-4 h-4" /><span>Edit Goal</span></button></li>
                        <li><button onClick={handleDelete} className="w-full text-left flex items-center gap-3 px-3 py-2 text-sm text-red-600 hover:bg-red-50"><Icons.Trash2Icon className="w-4 h-4" /><span>Delete Goal</span></button></li>
                    </ul>
                )}
            </div>
        );
                ...
                <div className="flex items-start gap-4 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-4 mt-2 flex-wrap">
                             ...
                             <button ref={triggerRefs.date} onClick={() => setPopup(p => p === 'date' ? null : 'date')} className="flex items-center gap-2 text-sm text-gray-600 hover:bg-gray-100 rounded px-2 py-1"><Icons.CalendarIcon className="w-4 h-4"/><span>{goal.targetEndDate ? `Target: ${new Date(goal.targetEndDate).toLocaleDateString()}` : 'Set Target Date'}</span></button>
                             {goalFocus && FocusIcon && (
                                <div className="flex items-center gap-2 text-sm font-medium bg-gray-100 rounded-full pl-1 pr-3 py-1">
                                    <FocusIcon className={`w-5 h-5 p-0.5 rounded-full bg-white text-${goalFocus.color}`} />
                    ...
                    <div className="p-4 bg-gray-50 rounded-lg space-y-3 border">
                        {goalLogs.length > 0 ? (
                            <ul className="space-y-2">
                                {goalLogs.map(log => <HabitLogItem key={log.id} log={log} />)}
                            </ul>
                        ) : (
                            <p className="text-sm text-gray-500 text-center py-2">No journal entries for this goal yet.</p>
                        )}
                    </div>
                </div>
