...
const QuickWinItem: React.FC<{
  quickWin: QuickWin;
  onStatusChange: (id: string, status: QuickWinStatus) => void;
}> = ({ quickWin, onStatusChange }) => {
      ...
      <div className="flex-1">
        ...
        {quickWin.dueDate && (
          <p className="text-xs text-gray-500 mt-0.5">
            Due: {new Date(quickWin.dueDate).toLocaleDateString()}
          </p>
        )}
      </div>
    </li>
...
const QuickWinList: React.FC<{
  quickWins: QuickWin[];
  onStatusChange: (id: string, status: QuickWinStatus) => void;
}> = ({ quickWins, onStatusChange }) => {
  const pending = quickWins
    .filter((qw) => qw.status === QuickWinStatus.PENDING)
    .sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime());
  const completed = quickWins
    .filter((qw) => qw.status === QuickWinStatus.COMPLETED)
    .sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime());
    ...
    <div className="w-full h-full p-4 overflow-y-auto">
      ...
      {pending.length > 0 ? (
        <ul className="space-y-2 mb-6">
          {pending.map((qw) => (
            <QuickWinItem key={qw.id} quickWin={qw} onStatusChange={onStatusChange} />
          ))}
        </ul>
      ) : (
