...
const getStatusColor = (status: MilestoneStatus) => {
    switch (status) {
        ...
        case MilestoneStatus.IN_PROGRESS: return 'bg-blue-100 text-blue-800';
        case MilestoneStatus.ABANDONED: return 'bg-red-100 text-red-800';
...
const MilestoneItem: React.FC<MilestoneItemProps> = ({ milestone, parentGoal, isSelected, onSelect, onEdit }) => {
    ...
    return (
        ...
        >
            <div className="flex justify-between items-start">
                ...
                <span className={`px-2 py-0.5 text-xs font-semibold rounded-full ${getStatusColor(milestone.status)} flex-shrink-0`}>{milestone.status}</span>
            </div>
...
const MilestoneList: React.FC<MilestoneListProps> = ({ milestones, goals, selectedMilestoneId, onSelectMilestone, activeFilter, onSetFilter, onAddMilestoneClick, onEditMilestone }) => {
    ...
    return (
        <div className="w-full h-full p-4 overflow-y-auto bg-gray-50/80 flex flex-col">
            <header className="flex justify-between items-center mb-4 flex-shrink-0">
                 ...
                 <button onClick={onAddMilestoneClick} className="flex items-center gap-2 px-3 py-1.5 bg-blue-600 text-white text-sm font-semibold rounded-lg hover:bg-blue-700 transition-colors shadow-sm">
