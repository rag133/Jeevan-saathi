...
const AddQuickWinForm: React.FC<AddQuickWinFormProps> = ({ onSave }) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            <div>
                <label htmlFor="qw-title" className="label">Title</label>
                <input id="qw-title" type="text" value={title} onChange={e => setTitle(e.target.value)} required className="input" placeholder="e.g., Reply to Sarah's email"/>
            </div>
            <div>
                <label htmlFor="qw-date" className="label">Due Date (Optional)</label>
                <input id="qw-date" type="date" value={dueDate} onChange={e => setDueDate(e.target.value)} className="input"/>
            </div>
            <div className="flex justify-end pt-4">
                <button type="submit" className="btn-primary">Add Quick Win</button>
            </div>
            <style>{`
