...
const AddGoalForm: React.FC<AddGoalFormProps> = ({ onSave, onDelete, initialGoal, allFoci }) => {
    ...
    const [focusAreaId, setFocusAreaId] = useState('');
     ...
     useEffect(() => {
        if (initialGoal) {
            ...
            setTargetEndDate(initialGoal.targetEndDate ? new Date(initialGoal.targetEndDate).toISOString().split('T')[0] : '');
    ...
    const handleDelete = () => {
        if (initialGoal && window.confirm(`Are you sure you want to delete the goal "${initialGoal.title}"? This will also delete all associated milestones.`)) {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            <div>
                ...
                <input id="goal-title" type="text" value={title} onChange={e => setTitle(e.target.value)} required className="input"/>
            </div>
            <div>
                <label htmlFor="goal-desc" className="label">Description</label>
                <textarea id="goal-desc" value={description} onChange={e => setDescription(e.target.value)} className="textarea" rows={3}/>
            </div>
            <div className="grid grid-cols-2 gap-4">
                <div>
                    <label htmlFor="goal-date" className="label">Target End Date (Optional)</label>
                    <input id="goal-date" type="date" value={targetEndDate} onChange={e => setTargetEndDate(e.target.value)} className="input"/>
                </div>
                <div>
                    <label htmlFor="goal-focus" className="label">Focus Area</label>
                    <select id="goal-focus" value={focusAreaId} onChange={e => setFocusAreaId(e.target.value)} className="input">
            ...
            <div className="flex justify-between items-center pt-4">
                ...
                <button type="submit" className="btn-primary">{initialGoal ? 'Save Changes' : 'Create Goal'}</button>
            </div>
            <style>{`
