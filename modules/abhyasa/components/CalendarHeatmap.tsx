...
const CalendarHeatmap: React.FC<CalendarHeatmapProps> = ({ habit, logs }) => {
    ...
    const monthLabels = weeks.reduce((acc, week, i) => {
        const firstDayOfMonth = week[0].getDate() === 1 || (i > 0 && weeks[i-1][0].getMonth() !== week[0].getMonth());
                    ...
                    {weeks.map((week, weekIndex) => (
                         <div key={weekIndex} className="flex flex-col gap-1">
                            {monthLabels.find(m => m.weekIndex === weekIndex) && (
                                <div className="text-xs text-gray-400 absolute -top-4" style={{left: `${weekIndex * 16}px`}}>
                                    {monthLabels.find(m => m.weekIndex === weekIndex)?.month}
                                </div>
                            )}
                            {week.map((day, dayIndex) => {
                                if (dayIndex % 2 === 0 && dayIndex !== 0) return null; // Only show Mon, Wed, Fri rows
                                if(day.getTime() > today.getTime()) return <div key={day.toISOString()} className="w-3 h-3 rounded-sm bg-transparent" />
