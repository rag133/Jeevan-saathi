...
const habitStatusItems: {id: HabitStatus, name: string, color: string, icon: keyof typeof Icons}[] = [
    { id: HabitStatus.YET_TO_START, name: 'Yet to Start', color: 'text-gray-500', icon: 'SunriseIcon' },
    { id: HabitStatus.IN_PROGRESS, name: 'In Progress', color: 'text-blue-500', icon: 'RepeatIcon' },
    { id: HabitStatus.COMPLETED, name: 'Completed', color: 'text-green-500', icon: 'CheckSquareIcon' },
    ...
    return (
        <div className="flex-1 bg-white p-6 flex flex-col h-full">
            <header className="flex justify-between items-start mb-6 pb-4 border-b border-gray-200">
                ...
                <button onClick={() => onEditHabit(habit)} className="p-2 hover:bg-gray-100 rounded-lg text-gray-500 hover:text-gray-800">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                     <div>
                        ...
                        <div className="mt-1 relative">
                            <button ref={triggerRef} onClick={() => setPopup(p => p === 'status' ? null : 'status')} className="flex items-center gap-2 text-sm font-medium bg-gray-100 hover:bg-gray-200 rounded-full px-3 py-1">
                    ...
                    <div>
                        <label className="text-sm font-medium text-gray-500">Log for {selectedDate.toLocaleDateString('en-US', { month: 'long', day: 'numeric' })}</label>
                    ...
                    <div className="p-4 bg-gray-50 rounded-lg space-y-3 border">
                        {habitLogsFromJournal.length > 0 ? (
                            <ul className="space-y-2">
                                {habitLogsFromJournal.map(log => <HabitLogItem key={log.id} log={log} />)}
                            </ul>
                        ) : (
                            <p className="text-sm text-gray-500 text-center py-2">No journal entries for this habit yet.</p>
                        )}
                    </div>
                </div>
