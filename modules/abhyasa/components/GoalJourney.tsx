...
const getMilestoneStatusClasses = (status: MilestoneStatus) => {
    switch (status) {
        case MilestoneStatus.COMPLETED:
            return { bg: 'bg-green-500', text: 'text-white', border: 'border-green-500', icon: 'CheckSquareIcon' };
        case MilestoneStatus.IN_PROGRESS:
            return { bg: 'bg-blue-500', text: 'text-white', border: 'border-blue-500', icon: 'RepeatIcon' };
        ...
        <div className={`flex items-center gap-3 p-2 rounded-lg bg-gray-50 border border-gray-200`}>
            ...
            <div className="flex-1 min-w-0">
                 ...
                 <div className="text-xs text-gray-500 flex items-center gap-1.5 mt-0.5">
                    ...
                    {habit.endDate && <> <Icons.ChevronRightIcon className="w-3 h-3"/> <span>{formatDate(habit.endDate)}</span></>}
                </div>
            </div>
            <div className={`text-xs font-semibold ml-auto px-2 py-1 rounded-full ${statusDetails.colorClass}`}>
...
const GoalJourney: React.FC<GoalJourneyProps> = ({
    ...
    const sortedMilestones = [...milestones].sort((a,b) => a.startDate.getTime() - b.startDate.getTime());
            ...
            {sortedMilestones.map((milestone) => {
                ...
                return (
                    <div key={milestone.id} className="relative mb-8">
                        {/* Milestone Node */}
                        <div className={`absolute left-4 top-2 w-8 h-8 rounded-full flex items-center justify-center transform -translate-x-1/2 ${statusClasses.bg}`}>
                            ...
                            <div className="flex justify-between items-start">
                                <div>
                                    ...
                                    <div className="text-xs text-gray-500 mt-1 flex items-center gap-2">
                                        ...
                                        <span>{milestone.targetEndDate ? formatDate(milestone.targetEndDate) : 'No Target Date'}</span>
                                    </div>
                                </div>
                                <span className={`px-2 py-0.5 text-xs font-semibold rounded-full bg-gray-100 text-gray-700`}>{milestone.status}</span>
                            </div>
                            
                            {milestone.description && <p className="text-sm text-gray-600 mt-2">{milestone.description}</p>}
                            
                            {(milestoneHabits.length > 0 || true) && (
                                <div className="mt-4 pt-3 border-t border-dashed">
                                    <h5 className="text-xs font-semibold text-gray-500 uppercase mb-2">Linked Habits</h5>
                                    <div className="space-y-2">
                                        {milestoneHabits.length > 0 ? (
                                            milestoneHabits.map(habit => <HabitPill key={habit.id} habit={habit} />)
                                        ) : (
                                            <p className="text-xs text-gray-500 italic py-1">No specific habits for this milestone.</p>
                                        )}
                                    </div>
                                    <div className="flex items-center gap-2 mt-3">
                                        <button onClick={() => onAddHabitToMilestone(milestone.id)} className="flex items-center gap-1 text-xs font-semibold text-blue-600 hover:text-blue-800 p-1 rounded hover:bg-blue-50"><Icons.PlusIcon className="w-3 h-3"/>Add Habit</button>
                                        <button onClick={() => onLinkHabitToMilestone(milestone.id)} className="flex items-center gap-1 text-xs font-semibold text-blue-600 hover:text-blue-800 p-1 rounded hover:bg-blue-50"><Icons.TagIcon className="w-3 h-3"/>Link Habit</button>
                                    </div>
                                </div>
                            )}
                        </div>
                 ...
                 <div className="ml-10 p-4 bg-gray-50 rounded-lg border border-gray-200">
                    ...
                    {habitsWithoutMilestone.length > 0 && (
                        <div className="mt-4 pt-3 border-t border-dashed">
                            ...
                            <div className="space-y-2">
                                {habitsWithoutMilestone.map(habit => <HabitPill key={habit.id} habit={habit} />)}
                            </div>
                        </div>
                    )}
                     ...
                     <div className="mt-4 pt-3 border-t border-dashed">
                        <h5 className="text-xs font-semibold text-gray-500 uppercase mb-2">Goal Actions</h5>
                         <div className="flex items-center gap-4 mt-2">
                            <button onClick={onAddMilestone} className="text-sm font-semibold text-gray-700 hover:text-black flex items-center gap-2"><Icons.PlusIcon className="w-4 h-4 text-white bg-gray-700 rounded-full p-0.5"/>Add Milestone</button>
                            <button onClick={onLinkMilestone} className="text-sm font-semibold text-gray-700 hover:text-black flex items-center gap-2"><Icons.TagIcon className="w-4 h-4"/>Link Milestone</button>
                            <button onClick={onAddHabit} className="text-sm font-semibold text-gray-700 hover:text-black flex items-center gap-2"><Icons.PlusIcon className="w-4 h-4 text-white bg-gray-700 rounded-full p-0.5"/>Add General Habit</button>
                            <button onClick={onLinkHabit} className="text-sm font-semibold text-gray-700 hover:text-black flex items-center gap-2"><Icons.TagIcon className="w-4 h-4"/>Link General Habit</button>
                        </div>
                    </div>
                </div>
            </div>
