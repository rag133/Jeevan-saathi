...
const getHabitStatusForToday = (habit: Habit, habitLogs: HabitLog[]): { text: string; color: string; icon: keyof typeof Icons } => {
...
const HabitPill: React.FC<{ habit: Habit; habitLogs: HabitLog[] }> = ({ habit, habitLogs }) => {
    ...
    return (
        <div className={`flex items-center gap-3 p-2 rounded-lg bg-gray-50 border border-gray-200`}>
            ...
            <div className={`flex items-center gap-1.5 text-xs font-semibold text-${todayStatus.color}-600 ml-auto px-2 py-1 rounded-full bg-${todayStatus.color}-100`}>
                <StatusIcon className={`w-3 h-3 ${todayStatus.icon === 'RepeatIcon' ? 'animate-spin' : ''}`} />
                    ...
                    <div className="mt-4 pt-3 border-t border-dashed">
                        {habits.length > 0 ? (
                             <div className="space-y-2">
                                {habits.map(habit => <HabitPill key={habit.id} habit={habit} habitLogs={habitLogs} />)}
                            </div>
                        ) : (
                            <p className="text-sm text-gray-500 italic py-1">No habits linked. Add habits to make progress on this milestone.</p>
                        )}
                    </div>
                     ...
                     <div className="mt-4 pt-3 border-t border-dashed">
                        <h5 className="text-xs font-semibold text-gray-500 uppercase mb-2">Actions</h5>
                         <div className="flex items-center gap-4 mt-2">
                            <button onClick={onAddHabit} className="text-sm font-semibold text-gray-700 hover:text-black flex items-center gap-2"><Icons.PlusIcon className="w-4 h-4 text-white bg-gray-700 rounded-full p-0.5"/>Add New Habit</button>
                            <button onClick={onLinkHabit} className="text-sm font-semibold text-gray-700 hover:text-black flex items-center gap-2"><Icons.TagIcon className="w-4 h-4"/>Link Existing Habit</button>
                        </div>
                    </div>
                </div>
            </div>
