    ...
    <>
      ...
      {modal && (
        <Modal title={modal === 'add-list' ? 'Create New List' : 'Create New Tag'} onClose={() => setModal(null)}>
            {modal === 'add-list' && <AddListForm folders={listFolders} onAddList={onAddList} onAddListFolder={onAddListFolder} />}
            {modal === 'add-tag' && <AddTagForm folders={tagFolders} onAddTag={onAddTag} onAddTagFolder={onAddTagFolder} />}
        </Modal>
      )}
      {logModalTask && (
        <LogEntryModal
