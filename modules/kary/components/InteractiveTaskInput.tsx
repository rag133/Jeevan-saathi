...
interface InteractiveTaskInputProps {
  lists: List[];
  tags: Tag[];
  onAddTask: (taskData: NewTaskData, list?: List | null) => Promise<void>;
}
...
const getPriorityData = (priority: 1 | 2 | 3 | 4) => {
  switch (priority) {
    case 1:
      return { text: '!Urgent', colorClass: 'bg-red-100 text-red-800', color: 'text-red-600' };
    case 2:
      return {
        text: '!High',
    ...
    case 3:
      return { text: '!Medium', colorClass: 'bg-blue-100 text-blue-800', color: 'text-blue-500' };
    case 4:
      return { text: '!Low', colorClass: 'bg-gray-100 text-gray-800', color: 'text-gray-500' };
  }
};
...
const Pill: React.FC<{
  ...
  colorClass: string;
  onRemove: () => void;
}> = ({ text, icon, colorClass, onRemove }) => (
  <span
    className={`flex items-center gap-1.5 rounded-md pl-2 pr-1 py-0.5 text-sm font-medium ${colorClass}`}
  >
...
const InteractiveTaskInput: React.FC<InteractiveTaskInputProps> = ({ lists, tags, onAddTask }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsCreating(true);
    try {
      const finalTaskData: NewTaskData = {
      ...
      await onAddTask(finalTaskData, selectedList);
      resetInput();
    } catch (error) {
  ...
  const handleSelect = (item: PopupItem) => {
    ...
    if (popup.isTextTriggered) {
      setText((prev) =>
        prev.substring(
          0,
          prev.lastIndexOf(popup.type === 'list' ? '~' : popup.type === 'tag' ? '#' : '!')
        )
      );
    }
    ...
    if (match) {
      ...
      if (trigger === '~' && !selectedList) {
        ...
        items = priorityItems.filter(
          (p) => p.id.includes(filter) || p.name.toLowerCase().includes(filter)
        );
      }
      ...
      >
        <div className="p-3">
          <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
            {priorityData && (
              <Pill
                text={priorityData.text.substring(1)}
                icon="FlagIcon"
                colorClass={priorityData.colorClass}
            ...
            {selectedList && (
              <Pill
                ...
                onRemove={() => setSelectedList(null)}
              />
            )}
            {selectedDueDate && (
              <Pill
                text={formatDatePill(selectedDueDate)}
                icon="CalendarIcon"
                colorClass="bg-blue-100 text-blue-800"
                onRemove={() => setSelectedDueDate(null)}
              />
            )}
            ...
            {selectedTags.map((tag) => (
              <Pill
                key={tag.id}
                text={tag.name}
                colorClass={`bg-${tag.color}/20 text-${tag.color}`}
                onRemove={() =>
          ...
          <div className="flex items-center gap-1 text-gray-500">
              ...
              >
                ...
                {selectedDueDate && (
                  <span className="text-xs text-blue-600 font-medium">
                    {formatDatePill(selectedDueDate)}
                  </span>
                )}
              </button>
              {isDatePickerOpen && (
            ...
            <button
              ...
              onClick={() =>
                openPopup(
                  'list',
                  lists.map((l) => ({ id: l.id, name: l.name, icon: l.icon, color: l.color }))
                )
              }
              className="flex items-center gap-1.5 p-1 rounded hover:bg-gray-200 transition-colors"
            >
              {selectedList ? (
                <>
                  {React.createElement(Icons[selectedList.icon] || Icons.ListIcon, {
                    className: `w-4 h-4 ${selectedList.color ? `text-${selectedList.color}` : ''}`,
                  })}
