...
const getCalendarGrid = (viewDate: Date | null | undefined): Date[] => {
    ...
    if (!viewDate || typeof viewDate !== 'object' || !(viewDate instanceof Date) || isNaN(viewDate.getTime())) {
        viewDate = new Date();
    }
        ...
        <div className="bg-white rounded-lg shadow-2xl border border-gray-200 w-80 p-4 font-sans text-gray-800">
            <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex items-center space-x-2 text-gray-500">
                    <button onClick={handlePrevMonth} className="p-1 rounded-full hover:bg-gray-100"><ChevronLeftIcon className="w-5 h-5" /></button>
                    <button onClick={handleGoToToday} title="Go to Today" className="w-5 h-5 border border-gray-400 rounded-full hover:bg-gray-100"></button>
                    <button onClick={handleNextMonth} className="p-1 rounded-full hover:bg-gray-100"><ChevronRightIcon className="w-5 h-5" /></button>
                </div>
            </div>

            <div className="grid grid-cols-7 gap-y-1 text-center text-sm">
                {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map(d => <div key={d} className="w-9 h-9 flex items-center justify-center text-gray-400 font-semibold">{d}</div>)}
                {calendarGrid.map((day, i) => {
                    const isCurrentMonth = day.getMonth() === viewDate.getMonth();
                    const isSelected = isSameDay(day, selectedDate);
                    const isToday = isSameDay(day, new Date());
                    
                    let buttonClass = "w-9 h-9 flex items-center justify-center rounded-full transition-colors duration-150";
                    if (isSelected) buttonClass += " bg-blue-600 text-white font-semibold";
                    else if (isToday) buttonClass += " border-2 border-blue-600 text-blue-600 font-semibold hover:bg-blue-50";
                    ...
                    return (
                        <div key={i} className="flex justify-center items-center">
                            <button onClick={() => handleDateClick(day)} className={buttonClass}>{day.getDate()}</button>
                        </div>
                    );
                })}
            </div>
                        ...
                        <div className="p-2 rounded-md bg-gray-50 border relative">
                             <div className="flex items-center gap-2">
                                ...
                                <div className="flex border border-gray-300 rounded-md ml-auto">
                                    <button type="button" onClick={() => setTimePeriod('AM')} className={`px-3 py-1 text-sm font-semibold rounded-l-md transition-colors ${timePeriod === 'AM' ? 'bg-blue-600 text-white' : 'bg-white text-blue-600 hover:bg-gray-100'}`}>AM</button>
                                    <button type="button" onClick={() => setTimePeriod('PM')} className={`px-3 py-1 text-sm font-semibold rounded-r-md transition-colors border-l border-gray-300 ${timePeriod === 'PM' ? 'bg-blue-600 text-white' : 'bg-white text-blue-600 hover:bg-gray-100'}`}>PM</button>
                                </div>
                            </div>
                            <button 
                                type="button" 
                            ...
                            {formattedTimeOptions.map(({ time, period }) => {
                                ...
                                return (
                                    <li key={`${time}-${period}`}>
                                        ...
                                        >
                                            <span>{time} <span className={isSelected ? 'text-blue-600' : 'text-gray-500'}>{period}</span></span>
            ...
            <div className="flex justify-end gap-2 mt-4">
                <button onClick={onClear} className="px-4 py-2 text-sm font-semibold text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-gray-300">Clear</button>
                <button onClick={handleOk} className="px-4 py-2 text-sm font-semibold text-white bg-blue-600 rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-blue-500">OK</button>
            </div>
        </div>
    );
};
