...
import { Focus, LogType, logTypeDetails, LogTemplate } from '~/modules/dainandini/types';
...
const AddFocusForm: React.FC<AddFocusFormProps> = ({ onSave, onDelete, initialData, allFoci }) => {
  ...
  const [allowedLogTypes, setAllowedLogTypes] = useState<LogType[]>([
    LogType.TEXT,
    LogType.CHECKLIST,
    LogType.RATING,
  ]);
  ...
  const handleDelete = () => {
    if (
      initialData &&
      window.confirm(
        `Are you sure you want to delete the "${initialData.name}" focus area? This cannot be undone.`
      )
    ) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div>
        <label htmlFor="focus-name" className="block text-sm font-medium text-gray-700">
          Focus Name
        </label>
        <input
          type="text"
      ...
      <div>
        <label htmlFor="focus-description" className="block text-sm font-medium text-gray-700">
        ...
        <div className="flex flex-wrap gap-4">
          {Object.values(LogType).map((type) => (
            <div key={type} className="flex items-center">
              ...
              />
              <label htmlFor={`log-type-${type}`} className="ml-2 text-sm text-gray-700">
      ...
      <div>
        <label htmlFor="focus-template" className="block text-sm font-medium text-gray-700">
        ...
        <p className="text-xs text-gray-500 mt-1">
          Assign a template to skip the selection step when adding a log to this focus area.
        </p>
      </div>
